package com.ayibonte.twitter.tasks;
...
public class LoadMoreAsyncTask extends AsyncTask<Void, Void, LoadMoreAsyncTask.LoadMoreStatusesResult> {
  ...
  protected LoadMoreStatusesResult doInBackground(Void...params) {
    ...
    try {
      if (newerThanId) {
          statii = (ArrayList<twitter4j.Status>) twitter.getHomeTimeline(new Paging(1).sinceId(targetId));
      } else {
        statii = (ArrayList<twitter4j.Status>) twitter.getHomeTimeline(new Paging(1).maxId(targetId));
      }
